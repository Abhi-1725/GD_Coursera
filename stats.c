...
#define SIZE 40
  ...
  {
  	unsigned char median_arrayElements=0;
    
    // if number of elements are even
    if(size%2 == 0)
        median_arrayElements = (array[size/2] + array[(size + 2)/2])/2.0;
  ...
  {
  	unsigned char sum = 0;
  	unsigned char mean_arrayElements = 0;
...
void main() 
{
  ...
  /* Other Variable Declarations Go Here */
	unsigned char median_arrayElements, mean_arrayElements, array_maxValue, array_minValue;
