...
float find_median(unsigned char* array, unsigned int size)
{
	...
	// declaring new array to store the sorted array for median calculation
	unsigned char array_for_median[SIZE];

	for (unsigned int i = 0; i < SIZE; ++i)
	{
		array_for_median[i] = array[i];
	}
	//sorting the array before calculating the median
	sort_array(array_for_median, SIZE);
	// if number of elements are even then considerng the ele
	if(size%2 == 0)
	    median_arrayElements = (array_for_median[size/2] + array_for_median[(size + 2)/2])/2;
	// if number of elements are odd
	else
	    median_arrayElements = array_for_median[size/2];
...
{
	unsigned int i=0 , j=0 , temp_element=0;
